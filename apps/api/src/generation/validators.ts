import { measurePlatformContent, platformLimit } from './social-post';
...
export function validateForPlatform(
  ...
  switch (platform) {
    case 'x': {
      ...
      const measured = measurePlatformContent(platform, content);
      if (measured > limit)
        problems.push(`超出 X 长度限制（加权 ${measured}/${limit}），请精简`)
