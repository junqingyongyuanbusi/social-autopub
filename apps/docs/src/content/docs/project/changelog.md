...
## 2026-09 — 文档补齐接入前准备与逐页操作指引

- 新增「接入前准备」分区五篇：准备清单、提交给管理员的信息清单、新手上手、Meta 开发者准备、X 开发者准备
- 补齐此前未覆盖的功能页：控制台「媒体工具」、「设置 → 用户管理」
- 控制台各页由「有什么」改写为「怎么做」：补入真实按钮文案、禁用原因提示、错误提示与对应处理
- 平台侧事实以 Postiz 官方 self-host 文档核对（Meta 应用模式与角色名单、IG 专业号与主页关联要求、X 必须选 Native App）
- 运维手册新增「运营反馈类问题对照表」；两个部署页补入平台开发者应用为前置条件的说明
